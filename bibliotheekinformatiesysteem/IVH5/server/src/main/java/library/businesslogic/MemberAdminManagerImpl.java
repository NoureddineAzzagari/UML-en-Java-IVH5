...
import library.datastorage.daofactory.rmi.RmiConnection;
 ...
 * @see library.main.LibraryServer
	...
	public ImmutableMember findMember(int membershipNumber) {
		...
		if (member == null) {
			...
			if (member != null) {
				...
				ReservationDAOInf reservationDAO = localDaoFactory
						.getReservationDAO();
				ArrayList<Reservation> reservations = reservationDAO
						.findReservations(member);
