...
/**
 * This test relies on the usage of stubs. The reason is twofold:
 * 1. the test is independent of the state of the database and can therefore
 *    easily be added to a regression test. In fact the test does not need
 *    a database to be present in order to succeed.
 * 2. to simulate error situations that cannot be created using the production
 *    code.
 * 
 * To have the test use the stubs
 * instead of the production code:
 * - go to "project properties"
 * - go to "libraries"
 * - go to tab "Run Tests"
 * - move "Compiled Tests" to the top
 * 
 * @author Erco
 */
public class MemberAdminManagerImplTest {
    ...
    private ImmutableMember removableMember;
    private ImmutableMember nonRemovableMember;
    private ImmutableMember problemMember;
    ...
    public void removeMember_path_1_4() {
        ...
        ImmutableMember member = memberAdminManager.findMember(1000); 
    ...
    public void removeMember_path_1_2_5() {
        ...
        ImmutableMember member = memberAdminManager.findMember(1002); 
